...
function renderPeriod(title, period) {
  ...
  return [
    ...
    ...(period.sampling ? [`- Sampling by section: total=${number(period.sampling.total || 1)}, pages=${number(period.sampling.pages || 1)}, channels=${number(period.sampling.channels || period.sampling.referrers || 1)}, referrers=${number(period.sampling.referrers || 1)}, flowSummary=${number(period.sampling.flowSummary || period.sampling.flows || 1)}, flowDetails=${number(period.sampling.flows || 1)}, entries=${number(period.sampling.entries || 1)}, countries=${number(period.sampling.countries || 1)}, devices=${number(period.sampling.devices || 1)}`] : []),
    ...(period.completeness ? [`- Row-limit coverage: pages=${period.completeness.pages !== false ? "below-cap" : "at-cap"}, channels=${(period.completeness.channels ?? period.completeness.referrers) !== false ? "below-cap" : "at-cap"}, referrers=${period.completeness.referrers !== false ? "below-cap" : "at-cap"}, flowSummary=${(period.completeness.flowSummary ?? period.completeness.flows) !== false ? "below-cap" : "at-cap"}, flowDetails=${period.completeness.flows !== false ? "below-cap" : "at-cap"}, entries=${period.completeness.entries !== false ? "below-cap" : "at-cap"}, countries=${period.completeness.countries !== false ? "below-cap" : "at-cap"}, devices=${period.completeness.devices !== false ? "below-cap" : "at-cap"}`] : []),
