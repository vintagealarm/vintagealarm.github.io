import profileWorker, { ENGLISH_GATEWAY_NAMES, GERMAN_GATEWAY_NAMES, patchAnalyticsPayload, patchDashboardHtml, WATCH_PAGE_NAMES, X_PROFILE_TRACKING } from './profile-worker.js';
...
assert(GERMAN_GATEWAY_NAMES['/de/pierce-duofon/'] === 'Pierce Duofon (DE)', 'Duofon German gateway mapping missing');
assert(GERMAN_GATEWAY_NAMES['/de/cyma-time-o-vox/'] === 'Cyma Time-O-Vox (DE)', 'Cyma German gateway mapping missing');
assert(GERMAN_GATEWAY_NAMES['/de/westclox-watchlarm/'] === 'Westclox Watchlarm (DE)', 'Westclox German gateway mapping missing');
...
assert(payload.current.snsEntries.pages.length === 16, 'SNS chart must contain five WATCH rows, six English rows, four German rows, plus Other pages');
...
assert(snsTotals['German Entry'] === 0, 'German index SNS row missing');
assert(snsTotals['Pierce Duofon (DE)'] === 0, 'Duofon German SNS row missing');
assert(snsTotals['Cyma Time-O-Vox (DE)'] === 0, 'Cyma German SNS row missing');
assert(snsTotals['Westclox Watchlarm (DE)'] === 0, 'Westclox German SNS row missing');
...
assert(patchedHtml.includes('["Basis Alarm","Pierce Duofon","Cyma Time-O-Vox","Citizen Alarm","Westclox Watchlarm","Basis Alarm (EN)","Pierce Duofon (EN)","Cyma Time-O-Vox (EN)","Citizen Alarm (EN)","Westclox Watchlarm (EN)","German Entry","Pierce Duofon (DE)","Cyma Time-O-Vox (DE)","Westclox Watchlarm (DE)"]'), 'WATCH share list was not expanded to Japanese, English and German WATCH entries');
...
assert(patchedHtml.includes('{name:"German Entry",path:"/de/"}'), 'German entry key page was not injected');
assert(patchedHtml.includes('{name:"Pierce Duofon (DE)",path:"/de/pierce-duofon/"}'), 'Duofon German key page was not injected');
assert(patchedHtml.includes('{name:"Cyma Time-O-Vox (DE)",path:"/de/cyma-time-o-vox/"}'), 'Cyma German key page was not injected');
assert(patchedHtml.includes('{name:"Westclox Watchlarm (DE)",path:"/de/westclox-watchlarm/"}'), 'Westclox German key page was not injected');
...
assert(dashboardHtml.includes('["Basis Alarm","Pierce Duofon","Cyma Time-O-Vox","Citizen Alarm","Westclox Watchlarm","Basis Alarm (EN)","Pierce Duofon (EN)","Cyma Time-O-Vox (EN)","Citizen Alarm (EN)","Westclox Watchlarm (EN)","German Entry","Pierce Duofon (DE)","Cyma Time-O-Vox (DE)","Westclox Watchlarm (DE)"]'), 'real dashboard WATCH share list is not expanded for German gateway');
...
assert(dashboardHtml.includes('{name:"German Entry",path:"/de/"}'), 'real dashboard is missing German entry key page');
assert(dashboardHtml.includes('{name:"Pierce Duofon (DE)",path:"/de/pierce-duofon/"}'), 'real dashboard is missing Duofon German key page');
assert(dashboardHtml.includes('{name:"Cyma Time-O-Vox (DE)",path:"/de/cyma-time-o-vox/"}'), 'real dashboard is missing Cyma German key page');
assert(dashboardHtml.includes('{name:"Westclox Watchlarm (DE)",path:"/de/westclox-watchlarm/"}'), 'real dashboard is missing Westclox German key page');
...
console.log('Five-WATCH analytics + English/German gateways + SNS reallocation + X profile attribution + AI readable URL wrapper: OK');