...
重要: この文書は実装そのものの正本ではありません。実装状態は常にGitHub `main` と対象ファイルを確認してください。ここには、現在の目的、判断基準、変更禁止事項、失効済み仕様、未解決事項、完了条件を短く保持します。
...
1. GitHub `main` の実装・対象ファイル
2. この `PROJECT_STATE.md` の現在判断・ルーティング
3. 分野別ルール / 方針
4. 計測・実験ログ
5. Git履歴・過去監査文書
6. 会話記憶
...
`MIGRATION.md`、`REFACTOR_AUDIT.md`、日付付き監査記録は履歴資料です。現在仕様と衝突する場合は、現在の `main` と上記正本を優先します。
...
この一覧だけで対象ページの実装状態を断定しない。編集前に必ず対象ファイルを確認する。
...
調査 → 対象特定 → 差分編集 → 必要な検証 → diff確認 → 必要なら公開確認 → 必要なら成果観測
