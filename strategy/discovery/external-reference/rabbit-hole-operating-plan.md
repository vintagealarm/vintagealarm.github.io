...
## 7. 現代語への翻訳層 — #勝手にキャッチコピー / OWNER'S NOTE

VINTAGE ALARMの **#勝手にキャッチコピー** と **OWNER'S NOTE** は、史実を置き換える文章ではなく、古い機械を現代の感覚へ接続するための翻訳層とする。

役割を分ける。

- **#勝手にキャッチコピー** — 一瞬で「何が変なのか」「今の何に似て見えるのか」を掴ませる入口。
- **OWNER'S NOTE** — 実機を持って触って鳴らした人間の視点から、その違和感をもう一段具体化する。
- **DEEP DIVE / SOURCES** — 機構・年代・資料差・一次資料・専門資料を確認する証拠層。

基本の流れは、

`現代語の違和感 → 実機の具体例 → OWNER'S NOTE → DEEP DIVE / SOURCES → 次の時計`

とする。

たとえば「1950年代のマナーモード」「身につける通知」「クロノメーターなのにアラーム」「腕時計に見えるが中身は別物」のような現代語は、**歴史的事実そのものとして書くのではなく、読者が古い機械の意味へ入るための比喩・入口**として扱う。

そのため、外部発見で現代語の問いを投げる時も、答えをキャッチコピーだけで完結させない。必ず実機・音・操作・資料へ戻れる構造にする。

**現代翻訳は入口、OWNER'S NOTEは接続、DEEP DIVE / SOURCESは根拠。**
