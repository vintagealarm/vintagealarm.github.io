...
HOW THEY RINGの上部は、ページ名・見出し・鳴動方式カード・方式ごとの歴史的な代表機だけで構成する。右側の編集意図説明やページ構成説明は置かない。代表機にはHISTORYで扱う著名機を使い、下段へ表示する所有・掲載個体と混同しない。BELLは、別体ベルをムーブメントに備えるETERNA ALARM / CAL.68を代表機とする。通常の歴史的Memovoxは裏蓋のピンを打つ構造なのでBELLへ分類しない。
