...
### 2026-09-24 13:03 JST — HOW THEY RING FIG.01 / FIG.03の発音経路を確定
- **変更**：FIG.01 OMEGA MEMOMATICを「棒状の音ばねを叩く」から **「輪状の音ばねを叩く」** へ変更。Cal.980はムーブメント固定の音ばねをハンマーが打撃し、その発音体は輪状であるため、旧留保を解消した。
- **変更**：FIG.03 JUNGHANS MINIVOXを「ピン／レバー伝達型」から **「ピン伝達型」** へ変更。量産型J89はハンマーがピンを打撃し、その振動を底部のベルへ伝えて鳴らすと整理した。
- **機構上の区別**：Basis Alarm / LANCO-FON Cal.1241はハンマーがベル自体を直接打撃する。Junghans J89はハンマーがピンを打撃してベルへ振動を伝える。FIG.03の「レバー」は発音伝達経路の名称としては使わない。
- **理由**：FIG.03で、試作時のレバー／量産型のベル／ピンによる伝達を同一の型名へ混在させていた。FIG.01も音ばね形状の確定後なお旧「棒状」と未解決留保が残っていた。
